...
const EditPromotion: React.FC = () => {
    ...
    const { register, handleSubmit, control, formState: { errors }, reset } = useForm<PromotionFormData>({
        defaultValues: {
            employeeId: '',
            fromPositionId: '',
            fromPositionName: '',
    ...
    useEffect(() => {
        const fetchData = async () => {
            try {
                ...
                // جلب الترقية
                const promoRes = await api.get(`/hr/promotions/${id}`);
                const promotionData = promoRes.data.promotion || promoRes.data;
                ...
                reset({
                    ...
                    fromPositionId: promotionData.fromPositionId || '',
                    fromPositionName: promotionData.fromPositionName || '',
                    toPositionId: promotionData.toPositionId || '',
                    toPositionName: promotionData.toPositionName || '',
                    fromSalary: promotionData.fromSalary ? Number(promotionData.fromSalary) : undefined,
                    toSalary: promotionData.toSalary ? Number(promotionData.toSalary) : undefined,
                    promotionDate: promotionData.promotionDate ? new Date(promotionData.promotionDate).toISOString().split('T')[0] : '',
                    effectiveDate: promotionData.effectiveDate ? new Date(promotionData.effectiveDate).toISOString().split('T')[0] : '',
                    reason: promotionData.reason || '',
                    notes: promotionData.notes || '',
                });
            } catch (error: any) {
                console.error('❌ Error fetching data:', error);
                const errorMessage = error.response?.data?.error || error.response?.data?.message || error.message || 'فشل في تحميل بيانات الترقية';
    ...
    const onSubmit = async (data: PromotionFormData) => {
        try {
            ...
            console.log('Updating promotion:', id, cleanedData);
            await api.put(`/hr/promotions/${id}`, cleanedData);
            toast.success('تم تحديث الترقية بنجاح');
            navigate('/hr/promotions');
        } catch (error: any) {
            console.error('❌ Error updating promotion:', error);
            const errorMessage = error.response?.data?.error || error.response?.data?.message || error.message || 'فشل في تحديث الترقية';
            toast.error(errorMessage);
        } finally {
            setSaving(false);
        }
            ...
            <div className="p-6">
                <Card>
                    <CardContent className="p-12 text-center">
                        <p className="text-gray-500">الترقية غير موجودة</p>
                        <Button variant="outline" onClick={() => navigate('/hr/promotions')} className="mt-4">
                            العودة للقائمة
                        </Button>
                    </CardContent>
                </Card>
            ...
            <div className="flex items-center justify-between">
                <div className="flex items-center gap-4">
                    ...
                    <div>
                        ...
                        <p className="text-gray-500 mt-1">تعديل بيانات الترقية</p>
                    </div>
                </div>
            </div>

/**
 * 🔧 Script to fix Foreign Key constraint for hr_resignations table
 * 
 * This script fixes the Foreign Key constraint on the userId column
 * in the hr_resignations table to properly reference the users table
 */

const { getSharedPrismaClient, initializeSharedDatabase } = require('../services/sharedDatabase');

async function fixResignationsForeignKey() {
  let prisma;
  ...
  try {
      ...
      for (const fk of existingFKs) {
        try {
          await prisma.$executeRawUnsafe(
            `ALTER TABLE \`hr_resignations\` DROP FOREIGN KEY \`${fk.CONSTRAINT_NAME}\``
          );
    ...
    if (columns.length === 0) {
      console.error('❌ userId column does not exist in hr_resignations table!');
    ...
    if (orphanedCountValue > 0) {
      ...
      const deleted = await prisma.$executeRawUnsafe(`
        DELETE r FROM hr_resignations r
        LEFT JOIN users u ON r.userId = u.id
        WHERE u.id IS NULL
      `);
    ...
    try {
      await prisma.$executeRawUnsafe(`
        ...
        ADD CONSTRAINT \`hr_resignations_userId_fkey\`
        FOREIGN KEY (\`userId\`)
        REFERENCES \`users\`(\`id\`)
        ON DELETE CASCADE
        ON UPDATE CASCADE
      ...
      if (error.message.includes('Duplicate') || error.message.includes('already exists')) {
        ...
        const orphanedRecords = await prisma.$queryRawUnsafe(`
          SELECT r.id, r.userId, r.companyId, r.resignationDate
          FROM hr_resignations r
          LEFT JOIN users u ON r.userId = u.id
          WHERE u.id IS NULL
          LIMIT 10
    ...
    console.log('\n📝 Summary:');
    console.log('   - Foreign Key constraint on hr_resignations.userId → users.id (ON DELETE CASCADE)');
...
module.exports = { fixResignationsForeignKey };

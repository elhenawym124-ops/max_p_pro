 ...
 * 1. تغيير نوع حقل usage إلى TEXT
 * 2. إصلاح جميع النماذج المقطوعة
 */
...
async function fixAllIssues() {
    try {
        ...
        // الخطوة 1: تغيير نوع حقل usage إلى TEXT
        console.log('📝 الخطوة 1: تغيير نوع حقل usage إلى TEXT...\n');
        ...
        try {
            ...
            console.warn(`⚠️ تحذير في تغيير نوع الحقل: ${error.message}`);
            console.log('   (قد يكون الحقل بالفعل TEXT أو يحتاج إلى migration)\n');
        }
        ...
        for (const modelRecord of allModels) {
            try {
                ...
                // إذا نجح التحليل، تحقق من الطول
                if ((modelRecord.usage || '').length <= 191) {
                    // على الأرجح مقطوع - إصلاحه
                    console.log(`🔧 إصلاح النموذج (طول قصير): ${modelRecord.model}`);

/**
 * 🔧 Script to fix Foreign Key constraint for hr_employee_warnings table
 * 
 * This script fixes the Foreign Key constraint on the userId column
 * in the hr_employee_warnings table to properly reference the users table
 */

const { getSharedPrismaClient, initializeSharedDatabase } = require('../services/sharedDatabase');

async function fixEmployeeWarningsForeignKey() {
  let prisma;
  ...
  try {
      ...
      for (const fk of existingFKs) {
        try {
          await prisma.$executeRawUnsafe(
            `ALTER TABLE \`hr_employee_warnings\` DROP FOREIGN KEY \`${fk.CONSTRAINT_NAME}\``
          );
    ...
    if (columns.length === 0) {
      console.error('❌ userId column does not exist in hr_employee_warnings table!');
    ...
    const orphanedCount = await prisma.$queryRawUnsafe(`
      ...
      FROM hr_employee_warnings ew
      LEFT JOIN users u ON ew.userId = u.id
      WHERE u.id IS NULL
    `);
    ...
    if (orphanedCountValue > 0) {
      ...
      const deleted = await prisma.$executeRawUnsafe(`
        DELETE ew FROM hr_employee_warnings ew
        LEFT JOIN users u ON ew.userId = u.id
        WHERE u.id IS NULL
      `);
    ...
    try {
      await prisma.$executeRawUnsafe(`
        ...
        ADD CONSTRAINT \`hr_employee_warnings_userId_fkey\`
        FOREIGN KEY (\`userId\`)
        REFERENCES \`users\`(\`id\`)
        ON DELETE CASCADE
        ON UPDATE CASCADE
      ...
      if (error.message.includes('Duplicate') || error.message.includes('already exists')) {
        ...
        const orphanedRecords = await prisma.$queryRawUnsafe(`
          SELECT ew.id, ew.userId, ew.companyId, ew.title
          FROM hr_employee_warnings ew
          LEFT JOIN users u ON ew.userId = u.id
          WHERE u.id IS NULL
          LIMIT 10
    ...
    console.log('\n📝 Summary:');
    console.log('   - Foreign Key constraint on hr_employee_warnings.userId → users.id (ON DELETE CASCADE)');
...
module.exports = { fixEmployeeWarningsForeignKey };

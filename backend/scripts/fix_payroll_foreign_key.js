/**
 * 🔧 Script to fix Foreign Key constraint for hr_payroll table
 * 
 * This script fixes the Foreign Key constraint on the userId column
 * in the hr_payroll table to properly reference the users table
 */

const { getSharedPrismaClient, initializeSharedDatabase } = require('../services/sharedDatabase');

async function fixPayrollForeignKey() {
  let prisma;
  ...
  try {
      ...
      for (const fk of existingFKs) {
        try {
          await prisma.$executeRawUnsafe(
            `ALTER TABLE \`hr_payroll\` DROP FOREIGN KEY \`${fk.CONSTRAINT_NAME}\``
          );
    ...
    if (columns.length === 0) {
      console.error('❌ userId column does not exist in hr_payroll table!');
    ...
    try {
      await prisma.$executeRawUnsafe(`
        ...
        ADD CONSTRAINT \`hr_payroll_userId_fkey\`
        FOREIGN KEY (\`userId\`)
        REFERENCES \`users\`(\`id\`)
        ON DELETE CASCADE
        ON UPDATE CASCADE
      ...
      if (error.message.includes('Duplicate') || error.message.includes('already exists')) {
        ...
        const orphanedRecords = await prisma.$queryRawUnsafe(`
          SELECT lr.id, lr.userId, lr.companyId, lr.month, lr.year
          FROM hr_payroll lr
          LEFT JOIN users u ON lr.userId = u.id
          WHERE u.id IS NULL
          LIMIT 10
    ...
    console.log('\n📝 Summary:');
    console.log('   - Foreign Key constraint on hr_payroll.userId → users.id (ON DELETE CASCADE)');
...
module.exports = { fixPayrollForeignKey };
